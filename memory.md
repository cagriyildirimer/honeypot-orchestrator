...
133. **Tehdit İstihbaratı (Threat Intel) Mükerrer API Sorguları Engellendi:** `database.go` içerisindeki `GetThreatIntelBulk` fonksiyonundan `INTERVAL '1 hour'` önbellek zamanaşımı sınırı kaldırıldı. Veritabanına (`threat_intel_cache`) bir kez kaydedilen IP'lerin AbuseIPDB, GreyNoise ve GeoIP API'lerine tekrar tekrar sorgu atması engellendi. Ayrıca `enricher.go` içerisine `sync.Map` tabanlı `inFlightEnrichments` kilidi eklenerek eşzamanlı mükerrer API istekleri tamamen ortadan kaldırıldı.
