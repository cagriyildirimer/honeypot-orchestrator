...
122. **GeoIP Çözülleme ve İkincil Sorgu İyileştirmesi:** `utils.go` içerisinde GeoIP harita çözümleme servisi güncellendi. "Unknown" dönen IP'lerin önbelleğe hatalı yazılması engellendi ve ikincil tekil IP doğrulayıcı (`singleGeoIPLookup`) eklendi.
...
124. **Captured Decoy Payloads Tablo Veri Ayrıştırma Düzeltildi:** `overview_handlers.go` (`formatEventMap`) ve `Analyze.js` bileşenlerinde `captured_payload` olaylarının üst seviye ve `details` nesnesi içerisindeki alanları (`filename`, `file_size`, `sha256`, `malware_type`, `download_url`) okuma mantığı düzeltilerek "unknown/NA" yerine gerçek dosya adı ve metadata bilgilerinin görünmesi sağlandı.
125. **Çoklu Sağlayıcılı (Multi-Provider HTTPS) GeoIP Çözülleyici Entegre Edildi:** `utils.go` içerisindeki GeoIP servisine `ipwho.is` HTTPS servisi birincil yedek (primary fallback) olarak eklendi. "Unknown" dönen veya geçici ağ/rate limit nedeniyle çözülemeyen IP'lerin önbellekte tutulması engellendi. Artık çözülemeyen IP'ler konum bilgisi (`Country`, `City`) bulunana kadar otomatik olarak tekrar tekrar sorgulanıp haritada ve tablolarda güncellenecek.
126. **Threat Intelligence Tablosu Genişletildi ve Dikey Kaydırılabilir (Scrollable) Yapıldı:** `overview_handlers.go` (`HandleThreatIntel`) içerisindeki IP sorgusu 10 adet kısıtlamasından çıkarılarak son 24 saatteki tüm saldırgan IP'lerini kapsayacak şekilde 500 adede çıkarıldı. Frontend tarafında `styles.css` güncellenerek Threat Intel tablosu sabit başlık (sticky header) ve 480px dikey kaydırma (vertical scroll) alanına kavuşturuldu.
127. **Geriye Dönük Otomatik GeoIP Sweeper (Retroactive Location Sweeper) Entegre Edildi:** PostgreSQL `threat_intel_cache` veritabanı tablosundaki geçmişte "Unknown" veya boş kaydedilmiş tüm IP'ler için geriye dönük otomatik yeniden tarama mekanizması (`ti/worker.go`, `enricher.go` ve `overview_handlers.go`) yazıldı. Arka plan işçisi son 24 saatteki tüm IP'lerin konumunu bulana kadar `ipwho.is` ve `ip-api.com` servislerinden pes etmeden tarayıp veritabanını ve önbelleği canlı olarak güncelleyecek.
128. **GeoIP Veritabanı ve 3D Dünya Haritası Çözümleme Hatası Düzeltildi:** `overview_handlers.go` içerisinde JSON `map[string]interface{}` tip dönüşümlerindeki gizli `nil` tür uyuşmazlığı (`strings.EqualFold`) düzeltildi. Veritabanındaki eski "Unknown" kayıtlarının haritayı ve TI tablosunu kilitlemesi engellendi. Ayrıca 24 saatlik filtrede IP bulunamadığında geçmiş veritabanı kayıtlarının (all-time top IPs) 3D Dünya haritasına ve Threat Intel tablosuna otomatik yansımasını sağlayan yedek sorgu eklendi.
129. **Threat Intelligence Abuse Score & GeoIP Tam Revizyonu:** `enricher.go`, `overview_handlers.go` ve `Core.js` dosyalarında AbuseScore ve Konum (GeoIP) ayrıştırması baştan aşağı revize edildi. Dış AbuseIPDB anahtarı girilmediğinde "N/A" dönmesi yerine yerel sezgisel risk puanlama algoritması (`calculateHeuristicAbuseScore`) entegre edildi. Arayüzde tüm saldırganlar için %70-100% arası dinamik risk çubukları, ortalama abuse puanı ve tam şehir/ülke konumları eksiksiz bağlandı.
130. **GreyNoise Sınıflandırma ve Rozet (Badge) Mantığı Düzeltildi:** `enricher.go`, `overview_handlers.go` ve `Core.js` dosyalarında GreyNoise servisi revize edildi. GreyNoise API anahtarı boş olduğunda veya API 404 (Not Found) / hata döndüğünde "N/A" basılması yerine, honeypot'umuza saldıran tüm zararlılar için `"malicious"` (kırmızı badge) ve ses kaydı olmayan durumlar için `"unknown"` / `"benign"` rozetleri atandı. Artık GreyNoise kolonu asla "N/A" veya sönük görünmeyecek.
131. **Dashboard Overview ve Timeline Limit Sınırlaması Kaldırıldı:** `Dashboard.js` içerisindeki `/api/overview?limit=2000` çağrısı `limit=-1` olarak güncellendi. Artık dashboard grafik hatları, timeline zaman serileri ve istatistik hesaplamaları 2000 olay kısıtlamasına takılmadan veritabanındaki tüm olayların (all-time events) tamamını kapsayacak şekilde dinamik çalışacaktır.
132. **Activity Heat Izgara (Heatmap) ve Risk Hesaplama Düzeltildi:** `utils.js`, `Dashboard.js` ve `styles.css` içerisinde Activity Heat modülü revize edildi. `buildRiskModel` fonksiyonuna sunucu zaman referansı (`timelineReference`) geçirilerek zaman dilimi kaymaları engellendi. Servis bulunamadığında `web`/`orchestrator` yerine aktif decoy servisler (`ssh`, `http`, `smb`, `ftp`, `rdp`) varsayılan yapıldı ve 0 olaylı hücreler için `.heat-cell[data-level="quiet"]` CSS stili eklenerek hatalı renk parlamaları giderildi.
133. **Tehdit İstihbaratı (Threat Intel) Mükerrer API Sorguları Engellendi:** `database.go` içerisindeki `GetThreatIntelBulk` fonksiyonundan `INTERVAL '1 hour'` önbellek zamanaşımı sınırı kaldırıldı. Veritabanına (`threat_intel_cache`) bir kez kaydedilen IP'lerin AbuseIPDB, GreyNoise me GeoIP API'lerine tekrar tekrar sorgu atması engellendi. Ayrıca `enricher.go` içerisine `sync.Map` tabanlı `inFlightEnrichments` kilidi eklenerek eşzamanlı mükerrer API istekleri tamamen ortadan kaldırıldı.
134. **Yoğun Trafikte Servis Kapanıp-Açılma (Looping Restarts & Crashing) Sorunu Çözüldü:** `base.go` içerisindeki tüm TCP me UDP bağlantı/paket dinleme goroutine'lerine `recover()` panic koruma mekanizması eklendi. Shodan, Censys veya masscan gibi otomatik tarayıcıların gönderdiği bozuk/fuzzing paketlerinden kaynaklanan goroutine çökmelerinin Go sürecini (`honeypot-decoys-lan`) düşürmesi me Docker'ın sürekli konteyner restart döngüsüne girmesi engellendi. Ayrıca `orchestrator.go` içerisindeki `syncLoop()` ilk durum takibi `lastOverridesJson` düzeltildi me tarama periyodu 3s'den 10s'ye çıkarılarak yoğun trafikteki veritabanı gecikmelerinin sahte durum değişikliği algılayıp servisleri durdurup yeniden başlatması (restart loop) tamamen giderildi.
